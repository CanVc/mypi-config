...
- **Task-State Gate:** Any context-discovery sub-agent launch MUST follow `.pi/skills/bmad-orchestrator/SKILL.md` `Task Routing and Task List State`. Maintain an orchestrator-managed task list in the active story/spec/run artifact when one exists; before `{spec_file}` exists, use a named workflow-run Markdown artifact such as `{implementation_artifacts}/epic-<N>-context-task-state.md`. Before dispatch, validate context and write the task to `in-progress` with `activeAgentId`; after parent validation write it to `completed`, or to `blocked` or `failed` with `cause` and `recommendedNextAction`; do not dispatch dependent tasks after a blocked/failed task.
   ...
   - **Determine context strategy.** Using the intent and the artifact listing, infer whether the current work is a story from an epic. Do not rely on filename patterns or regex — reason about the intent, the listing, and any epics file content together.
     ...
     **A) Epic story path** — if the intent is clearly an epic story:
     ...
     3. **Compile epic context.** Produce `{implementation_artifacts}/epic-<N>-context.md` by following `./compile-epic-context.md`, in order of preference:
        ...
        - **Fallback — inline** (for runtimes without sub-agent support, e.g. Copilot, Codex, local Ollama, older Claude):
          - If the runtime cannot spawn sub-agents at all, record the route as inline context compilation in the task list or workflow-run notes, then read `./compile-epic-context.md` yourself and follow its instructions to produce the same output file.
          - If the spawn fails or times out after a context-discovery task was attempted, first write that attempted sub-agent task to durable `blocked` or `failed` state with `cause` and `recommendedNextAction` in the orchestrator-managed task list. Continuing inline is an explicit recovery, not an implicit fallback: record either a separate recovery task or a `routingDecision` that names the failed/timeout attempt, explains why inline recovery is authorized, and declares the same artifact-first context source. Do not treat failed or timed-out spawn fallback as the same task continuing normally.
     ...
     5. **Previous story continuity.** Regardless of which context source succeeded above, scan `{implementation_artifacts}` recursively for quick-dev specs and folder-based BMAD story files from the same epic with `status: done` / `Status: done` and a lower story number. Prefer canonical BMAD story files at `{implementation_artifacts}/<story-key>/<story-key>.md`, but support legacy flat `{implementation_artifacts}/<story-key>.md` files. Load the most recent one (highest story number below current). For quick-dev specs, extract **Code Map**, **Design Notes**, **Spec Change Log**, and **task list**; for BMAD story files, extract **Dev Notes**, **Dev Agent Record**, **File List**, **Change Log**, and review notes as continuity context for step-02 planning. If no `done` artifact is found but an `in-review`/`review` artifact exists for the same epic with a lower story number, note it to the user and ask whether to load it.
