...
1. **Find the review target.** The conversation context before this skill was triggered IS your starting point — not a blank slate. Check in this order — stop as soon as the review target is identified:
   ...
   **Tier 3 — Sprint tracking.**
   ...
   - **Exactly one `review` story:** Set `{story_key}` to the story's key (e.g., `1-2-user-auth`). Resolve `{spec_file}` by first checking `{implementation_artifacts}/{story_key}/{story_key}.md`, then falling back to legacy `{implementation_artifacts}/{story_key}.md` only for backward compatibility. Set `{review_artifact_dir}` to `{implementation_artifacts}/{story_key}` whenever `{story_key}` is known, even if `{spec_file}` resolved through the legacy flat fallback. Suggest it: "I found story <story-id> in `review` status. Would you like to review its changes? [Y] Yes / [N] No, let me choose". If confirmed, use the story context to determine the diff source (branch name derived from story slug, or uncommitted changes). If declined, clear `{story_key}`, `{spec_file}`, and `{review_artifact_dir}` and fall through.
   - **Multiple `review` stories:** Present them as numbered options alongside a manual choice option. Wait for user selection. If a story is selected, set `{story_key}`, resolve `{spec_file}` by first checking `{implementation_artifacts}/{story_key}/{story_key}.md` with legacy flat fallback, set `{review_artifact_dir}` to `{implementation_artifacts}/{story_key}` whenever `{story_key}` is known, and use its context to determine the diff source. If manual choice is selected, clear `{story_key}` and fall through.
   - **None:** Fall through.
